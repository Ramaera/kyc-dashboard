...
export const rows = [
  {
    config: documentsConfig.upgrade_agency_license_payment_proof_documents
  }
];
const DocumentRow = ({
  data,
...
const UpgradeAgencyPayment = () => {
  ...
  const [upgradeAgencyPaymentDocument, setUpgradeAgencyPaymentDocument] =
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      if (upgradeAgencyPaymentDocument) {
        await updateDocument({
          variables: {
            title: documentsConfig.agency_payment_proof.items[0].id,
            url: imgUrl,
            id: upgradeAgencyPaymentDocument.id
          }
        });
        setSubmitButtonEnabled(false);
        dispatch(
          setOrUpdateUser(updateUser(upgradeAgencyPaymentDocument.id, imgUrl))
        );
      } else {
    ...
    if (user && user?.documents && user?.documents?.length > 0) {
      user?.documents?.find((document: DocumentType) => {
        if (
          ...
          setUpgradeAgencyPaymentDocument(document);
  ...
  return (
    <>
      {!additionalDocuments ? (
        <>
          <PaymentInfo docStatus={upgradeAgencyPaymentDocument?.status} />
          ...
          {upgradeAgencyPaymentDocument && upgradeAgencyPaymentDocument.status && (
            <Typography variant="h4" sx={{ my: 2 }}>
              Status :{' '}
              <span
                style={{
                  color: upgradeAgencyPaymentDocument
                    ? (upgradeAgencyPaymentDocument.status === 'APPROVED' &&
                        'green') ||
                      (upgradeAgencyPaymentDocument.status === 'REJECTED' &&
                        'red')
                    : ''
                }}
              >
                {upgradeAgencyPaymentDocument &&
                  upgradeAgencyPaymentDocument.status}
              </span>
            </Typography>
          )}
          {documentStatus && (
            <Grid container py={2} spacing={2}>
              <Grid item xs={12} sm={5} md={3} lg={3}>
                <Button
                  ...
                  component="label"
                  style={{
                    cursor: upgradeAgencyPaymentDocument
                      ? upgradeAgencyPaymentDocument.status === 'APPROVED'
                        ? 'not-allowed'
                        : 'pointer'
                      : 'pointer'
...
export default UpgradeAgencyPayment;
